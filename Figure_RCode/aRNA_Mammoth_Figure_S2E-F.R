...
data_somatic$alpha = ifelse(data_somatic$counts > 11, "#5D987B", "#D9E8E1")
data_sex$alpha = ifelse(data_sex$counts > 4, "#5D987B", "#D9E8E1")
...
datar_somatic$alpha = ifelse(datar_somatic$counts > 10, "#4D4DA1", "grey85")
datar_sex$alpha = ifelse(datar_sex$counts > 5, "#4D4DA1", "grey85")
...
#Figure S2E
pdf("aRNA_Mammoth_Figure_S2E.pdf", height = 60/25.4, width = 165/25.4)
ggplot() + 
  geom_point(data = data, aes(x = pos, y = log10(as.numeric(counts)+1), fill=Type), color = data$alpha, size=2, alpha=0.7) +
  geom_point(data = datar, aes(x = pos, y = log10(as.numeric(counts)+1), fill=Type), color = datar$alpha, size=2, alpha=0.7) +
  
  geom_segment(aes(x = 1700000000, y = 3.3, xend = 2640000000, yend = 3.3), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_segment(aes(x = 1700000000, y = 4.6, xend = 2640000000, yend = 4.6), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_segment(aes(x = 1700000000, y = 3.3, xend = 1700000000, yend = 4.6), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_segment(aes(x = 2640000000, y = 3.3, xend = 2640000000, yend = 4.6), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_text(aes(label="rRNA gene\n clusters", x=2150000000, y=4, label="Center"), color = "black", size=2.5) +
  ...
  guides(fill = guide_legend(override.aes=list(shape=19, alpha= 0.5, colour=c("#4D4DA1", "#5D987B"), size=4), nrow=2)) +
  ...
  theme(axis.text.x = element_text(size=7, angle=45, vjust=1, hjust=1),
        ...
        plot.margin = margin(t=0.25, b=0.1, l=0.25, r=0.25, "cm")) +
  
  ggtitle("Mammoth 4 Genome-wide alignment") + theme(plot.title = element_text(size = 8)) +
...
data_Y =  subset(data, data$chr_alt == "ChrY")
data_sex = data_X
data_somatic$alpha = ifelse(data_somatic$counts > 128, "#5D987B", "#D9E8E1")
data_sex$alpha = ifelse(data_sex$counts > 128, "#5D987B", "#D9E8E1")
data = rbind(data_somatic, data_sex)
data$Type = rep("aRNA", nrow(data))
...
datar_Y =  subset(datar, datar$chr_alt == "ChrY")
datar_sex = datar_X
datar_somatic$alpha = ifelse(datar_somatic$counts > 128, "#4D4DA1", "grey85")
datar_sex$alpha = ifelse(datar_sex$counts > 128, "#4D4DA1", "grey85")
datar = rbind(datar_somatic, datar_sex)
datar$Type = rep("aDNA", nrow(datar))

chr_alt = chr_alt[1:28]
namef = factor(chr_alt, levels = chr_alt, ordered = TRUE)
...
mean_pos2 = c(122250000, 362750000, 592250000, 802000000, 984000000, 1139750000, 1281500000, 1418250000, 1549500000, 1673500000,
              1792500000, 1906750000, 2015500000, 2119000000, 2217500000, 2313500000, 2407250000, 2495750000, 2581250000, 2666500000, 
              2750750000, 2833750000, 2914750000, 2988750000, 3057250000, 3116250000, 3170000000, 3270000000)
...
pdf("aRNA_Mammoth_Figure_S2F.pdf", height = 60/25.4, width = 165/25.4)
ggplot() + 
  geom_point(data = data, aes(x = pos, y = log10(as.numeric(counts)+1), fill=Type), color = data$alpha, size=2, alpha=0.7) +
  geom_point(data = datar, aes(x = pos, y = log10(as.numeric(counts)+1), fill=Type), color = datar$alpha, size=2, alpha=0.7) +
  
  geom_segment(aes(x = 1700000000, y = 3.5, xend = 2640000000, yend = 3.5), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_segment(aes(x = 1700000000, y = 4.2, xend = 2640000000, yend = 4.2), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_segment(aes(x = 1700000000, y = 3.5, xend = 1700000000, yend = 4.2), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_segment(aes(x = 2640000000, y = 3.5, xend = 2640000000, yend = 4.2), linetype = "dashed", color = "black", linewidth = 0.3) +
  geom_text(aes(label="rRNA gene clusters", x=2150000000, y=3.9, label="Center"), color = "black", size=2.5) +
  ...
  ggtitle("Mammoth 10 Genome-wide alignment") + theme(plot.title = element_text(size = 8)) +
  theme(plot.title = element_text(face="bold"))
